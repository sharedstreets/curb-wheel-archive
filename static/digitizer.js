  ...
  ui: {
    entry: {
      ...
      applyPropagations: function (d, i) {
        ...
        app.constants.ui.entryPropagations.forEach((propagation) => {
          entry
            .select(`div[prop=${propagation.originProp}] select`)
            .on("change", updateDestination);

          function updateDestination(d, i) {
            var parentValue = d3.select(this).property("value");
            var propagatingRule =
              propagation.propagatingValues[parentValue] || false;
            var prop = entry.select(`div[prop=${propagation.destinationProp}]`);

            prop.classed("hidden", !propagatingRule);
