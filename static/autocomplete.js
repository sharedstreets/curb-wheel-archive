function autocomplete(inp, options) {
  ...
  function onInput() {
    var a,
      b,
      i,
      val = options.inputTransform
        ? options.inputTransform(this.value)
        : this.value;
    ...
    for (i = 0; i < options.values.length; i++) {
      var matchFound =
        options.match === "any"
          ? options.values[i].includes(val)
          : options.values[i].substr(0, val.length).toUpperCase() ==
            val.toUpperCase();
      ...
      if (matchFound) {
        ...
        b.addEventListener("click", function (e) {
          ...
          var valueToApply = options.outputTransform
            ? options.outputTransform(inp.value, selectedValue)
            : selectedValue;
