var app = {
	...
	updateCounter: () => {
	  ...
	  req.open('GET', '/counter/survey', true);
	  req.onload  = function() {
		  var jsonResponse = JSON.parse(req.responseText);
		  if(jsonResponse.timestamp > app.state.lastCounterTimestamp) {
			app.state.lastCounterTimestamp = jsonResponse.timestamp
			app.state.currentRollDistance = jsonResponse.counter;
			app.ui.roll();
		  }
	  };
	  req.send(null);
	...
	resetCounter: () => {
	  ...
	  req.open('POST', '/counter/survey/reset');
	  req.send(null);
	},
